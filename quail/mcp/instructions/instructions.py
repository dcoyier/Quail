...
def unrestricted_instructions(workspace_id: str) -> str:
    return (
        ...
        "search(session_id, dataset_id, query) → optional get_entry(...). "
        ...
        "(optional top_k, default 8, max 20). get_entry returns the full row "
        "for an entry_id from search hits. Do not invent evidence beyond tool results.\n"
        "\n"
...
def clerk_instructions(*, locked: bool = False) -> str:
    base = (
        ...
        "(optional get_entry). Prefer quail_setup over separately calling "
