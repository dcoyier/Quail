"""In-process lock: one analysis/search call at a time per session_id."""
...
_BUSY_REPAIR = (
    "Wait for the in-flight call on this session_id to finish, "
    "then retry. Do not overlap calls on the same session_id."
)
    ...
    with _lock:
        if key in _held:
            raise QuailSessionBusyError(
                "Another call is already running for this session_id.",
