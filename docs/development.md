...
| Analysis language / agent `quail_exec` contract | Served today: `docs/api.md`. Rebuild spec (unpublished, standalone): `docs/api_draft.md` — two verbs (`retrieve` / `count`); Python and `.map(fn)` build `group` / `rank` / `of`. |
