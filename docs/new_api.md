...
> **Draft (`new_api.md`, temporary).** Candidate rewrite of the document agents
> read before using `quail_exec`. Not served over MCP yet. Compare with
> `api.md` and keep what earns its place.
...
## Design notes (for humans comparing drafts)

Kept because they are the product:

- Symbolic → materialize → print
- Session overlays; immutable imports
- Regex + lexical + semantic + ranking
- `retrieve` / `count` / mutations / bindings

Tightened vs the long v0.10 doc:

- Lead with workflows, not an encyclopedia of every edge
- One short “core rules” section instead of repeating isolation five times
- Search/config failure described as “repair and rerun,” without hosting detail
- Dropped dual internal representations and operator-platform ceremony

Still open for you to decide when promoting this over `api.md`:
