...
def embed_batch(base_url: str, model: str, dimensions: int, texts: list[str], timeout: int = 600) -> list[list[float]]:
    ...
    if not isinstance(vectors, list) or len(vectors) != len(texts):
        raise RuntimeError(f"bad embeddings response size: {type(vectors)} len={getattr(vectors,'__len__',lambda:None)()}")
    return vectors
    ...
    with gzip.open(args.out, "wt", encoding="utf-8") as out:
        for i in range(0, len(rows), args.batch_size):
            ...
            for attempt in range(5):
                try:
                    vectors = embed_batch(args.base_url, args.model, args.dimensions, texts)
                    break
                except Exception as e:
                    wait = 2 ** attempt
                    print(f"batch {i} attempt {attempt+1} failed: {e}; sleep {wait}s", flush=True)
                    time.sleep(wait)
            else:
            ...
            for row, vec in zip(batch, vectors):
                ...
                out.write(json.dumps({"text_hash": row["text_hash"], "vector": vec}) + "\n")
            done += len(batch)
            elapsed = time.perf_counter() - t0
            rate = done / elapsed if elapsed else 0
            print(f"progress {done}/{len(rows)}  {rate:.1f}/s  elapsed={elapsed:.0f}s", flush=True)
    print(f"wrote {args.out}  total={done}  wall={time.perf_counter()-t0:.1f}s", flush=True)
